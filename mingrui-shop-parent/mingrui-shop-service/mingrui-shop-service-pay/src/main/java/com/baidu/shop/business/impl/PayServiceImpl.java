...
import com.baidu.shop.fegin.OrderFegin;
...
public class PayServiceImpl extends BaseApiService implements PayService {
    ...
    //请求支付
    @Override
    public void requestPay(PayinfoDTO payinfoDTO, String token, HttpServletResponse httpServletResponse) {
        ...
        Result<OrderInfo> orderResult = orderFegin.getOrderInfoByOrderId(payinfoDTO.getOrderId());
        if(orderResult.getCode() ==200){
            OrderInfo orderInfo = orderResult.getData();

            //获取订单名称
            List<String> titleList = orderInfo.getOrderDetailList().stream().map(detail -> detail.getTitle()).collect(Collectors.toList());

            String titles = String.join(",", titleList);
            titles = titles.length() > 10 ? titles.substring(0, 10) : titles;
            //商户订单号，商户网站订单系统中唯一订单号，必填
            String out_trade_no = payinfoDTO.getOrderId()+"";
            //付款金额，必填
            String total_amount = Double.valueOf(orderInfo.getActualPay()/100)+""; //数据库以分为单位 需要/100
            //订单名称，必填
            String subject =titles;
            //商品描述，可空
            String body ="";
            ...
            try {
                //拿到了PrintWriter 这个字符流，然后对流进行操作
                // 操作的结果反映到response对应的位置 out对象中 ?
